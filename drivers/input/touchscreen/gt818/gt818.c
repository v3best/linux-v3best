 ...
 *      V2.0:2014/12/15,LeeSheen Modify. Farsight FS4412 support
 *      
 */
...
#define dprintk(level_mask,fmt,arg...)    if(unlikely(debug_mask & level_mask)) \
        printk("***CTP***"fmt, ## arg)
//#define dprintk(level_mask,fmt,arg...)  printk(KERN_ERR "***CTP***"fmt, ## arg)
...
void gtp_sleep(int sleep)
{
	int ret;
	ret = gpio_request(57, "GPX0_3");
	if(ret)
	{
		printk(KERN_ERR "failed to request GPJ1_4 for LVDS PWDN pin\n");
	}

	//s3c_gpio_cfgpin(EXYNOS4_GPX0(3), S3C_GPIO_SFN(1));
	gpio_direction_output(57, sleep);
	gpio_free(57);
...
{	
	gtp_io_init(20);
    gtp_set_io_int();//add by hl 20131226
	gtp_sleep(0);
	msleep(ms);
	gtp_sleep(1);
...
#define ROW_MAX        1024
#define FW_MAX         256   
static  char *gt818_update_file_path = NULL;
...
//gt818 wj 20131105 800*480
u8 cfg_info_group1[106] = {
...
static int  ReadDataFromFile(char *update_file_path)
{
    struct file *fp; 
    mm_segment_t fs; 
    loff_t pos=0; 
    ...
    CTPM_FW=vmalloc(FW_MAX); 
    fp = filp_open(update_file_path, O_RDONLY, 0644); 
    if (IS_ERR(fp)) { 
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	ret = ReadDataFromFile(gt818_update_file_path);
	if(ret < 0)
		printk("Read Data From File failed,used default data!\n");
	
	//add by hl 20140106 Debug cfg_info_group1
	cfg_num = sizeof(cfg_info_group1)/sizeof(cfg_info_group1[0]);
#if 0
	printk("cfg_info_group1[%d]:",sizeof(cfg_info_group1)/sizeof(cfg_info_group1[0]));												
	for(i = 0; i< cfg_num; i++)	
		printk("0x%02x ",cfg_info_group1[i]);	
	printk("\n");	
#endif	
...
*******************************************************/
/*
*2013/12/26/V1.6: 更新升级IC固件后复位一下IC
*/
...
#define wake_pin              57
    ...
    ts->client->irq = gpio_to_irq(pen_irq_pin);
    /********************************************/
